...
async def send_doc(bot: Client, m: Message, cc, ka, cc1, prog, count, name):
    reply = await m.reply_text(f"**Fast Uploading..)\n\n {name}**")
    time.sleep(1)
...
async def send_vid(bot: Client, m: Message,cc,filename,thumb,name,prog, reply_markup=None):
    ...
    try:
        await m.reply_video(filename,caption=cc, supports_streaming=True,height=720,width=1280,thumb=thumbnail,duration=dur, progress=progress_bar,progress_args=(reply,start_time), reply_markup=reply_markup)
    except Exception:
        await m.reply_document(filename,caption=cc, progress=progress_bar,progress_args=(reply,start_time), reply_markup=reply_markup)
